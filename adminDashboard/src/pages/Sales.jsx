...
export default function Sales() {
  ...
  const chartOptions = {
    ...
    scales: {
      y: {
        grid: { color: '#c6d3e5ca' },
        ticks: { color: '#ffffff', callback: (val) => '$' + val.toLocaleString() }
      },
      x: {
        grid: { color: '#e9edf2' },
        ticks: { color: '#ffffff' }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-slate-900 rounded-3xl p-8" bg-white>
        <h3 className="text-xl font-semibold mb-6 text-white">Top Selling Products</h3>
        <table className="w-full">
          ...
          <tbody className="divide-y divide-slate-700">
            {topProducts.map((product, index) => (
              <tr key={index} className="hover:bg-slate-800/50 text-white">
