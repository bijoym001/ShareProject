  ...
  return (
    <div className="w-72 bg-white border-r border-gray-200 flex flex-col h-screen shadow-lg">
      ...
      <div className="p-6 border-b border-gray-200 flex items-center gap-3">
        <div className="w-10 h-10 from-blue-500 to-indigo-600 rounded-2xl flex items-center justify-center text-white font-bold text-3xl">
          A
        </div>
        <div>
      ...
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex bg-gray-100 rounded-3xl p-1 text-sm font-medium">
          <button
            ...
            className={`flex-1 py-2.5 rounded-3xl transition-all ${currentUser.role === 'admin' ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-200'}`}>
            ADMIN
          </button>
          <button
            onClick={() => switchRole('user')}
            className={`flex-1 py-2.5 rounded-3xl transition-all ${currentUser.role === 'user' ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-200'}`}>
            USER
          </button>
        </div>
      </div>
